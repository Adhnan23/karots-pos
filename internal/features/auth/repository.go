...
func (r *Repository) Create(ctx context.Context, name string, phone *string, role, pinHash, receiptPrinter string, mustChange, canSuppliers, canCredit, canSeeCost bool) (*User, error) {
	var u User
	err := r.db.GetContext(ctx, &u,
		`INSERT INTO users (name, phone, role, pin_hash, must_change_pin, receipt_printer, can_handle_suppliers, can_manage_credit, can_see_cost)
		 VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)
		 RETURNING *`, name, phone, role, pinHash, mustChange, receiptPrinter, canSuppliers, canCredit, canSeeCost)
	if err != nil {
		return nil, err
	}
	return &u, nil
...
func (r *Repository) Update(ctx context.Context, id int64, name string, phone *string, role, receiptPrinter string, canSuppliers, canCredit, canSeeCost bool) error {
	res, err := r.db.ExecContext(ctx,
		`UPDATE users SET name = $1, phone = $2, role = $3, receipt_printer = $4, can_handle_suppliers = $5, can_manage_credit = $6, can_see_cost = $7
		 WHERE id = $8 AND is_system = false`,
		name, phone, role, receiptPrinter, canSuppliers, canCredit, canSeeCost, id)
	if err != nil {
		return err
	}
