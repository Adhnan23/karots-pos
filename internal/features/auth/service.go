...
func (s *Service) CreateUser(ctx context.Context, in CreateUserInput) (*User, error) {
	...
	u, err := s.repo.Create(ctx, in.Name, &in.Phone, in.Role, string(hash), in.ReceiptPrinter, s.forcePinChange(ctx), checkboxOn(in.CanHandleSuppliers), checkboxOn(in.CanManageCredit))
	if err != nil {
...
func (s *Service) UpdateUser(ctx context.Context, id int64, in UpdateUserInput) error {
	err := s.repo.Update(ctx, id, in.Name, &in.Phone, in.Role, in.ReceiptPrinter, checkboxOn(in.CanHandleSuppliers), checkboxOn(in.CanManageCredit))
	if errors.Is(err, sql.ErrNoRows) {
